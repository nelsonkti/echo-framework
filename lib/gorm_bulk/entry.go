...
func BulkInsert(db *gorm.DB, objects []interface{}) error {
    return BulkExec(db, objects, gormbulk.InsertFunc)
}

// BulkInsertIgnore will call BulkExec with the default InsertFunc.
func BulkInsertIgnore(db *gorm.DB, objects []interface{}) error {
    return BulkExec(db, objects, gormbulk.InsertIgnoreFunc)
}

// BulkInsertOnDuplicateKeyUpdateTotalMinutes will call BulkExec with the default InsertFunc.
func BulkInsertOnDuplicateKeyUpdate(db *gorm.DB, objects []interface{}) error {
    return BulkExec(db, objects, gormbulk.InsertOnDuplicateKeyUpdateFunc)
}

// BulkExecChunk will split the objects passed into the passed chunk size. A
// slice of errors will be returned (if any).
func BulkExecChunk(db *gorm.DB, objects []interface{}, execFunc gormbulk.ExecFunc, chunkSize int) []error {
    var allErrors []error
...
func BulkExec(db *gorm.DB, objects []interface{}, execFunc gormbulk.ExecFunc) error {
...
func scopeFromObjects(db *gorm.DB, objects []interface{}, execFunc gormbulk.ExecFunc) (*gorm.Scope, error) {
...
// BulkInsertIgnore will call BulkExec with the default InsertFunc.
func BulkInsertIgnoreWithColumns(db *gorm.DB, objects []interface{}, columnNames []string) error {
    return BulkExecWithColumns(db, objects, gormbulk.InsertIgnoreFunc, columnNames)
}


func BulkInsertWithColumns(db *gorm.DB, objects []interface{}, columnNames []string) error {
    return BulkExecWithColumns(db, objects, gormbulk.InsertFunc, columnNames)
}

func BulkExecWithColumns(db *gorm.DB, objects []interface{}, execFunc gormbulk.ExecFunc, columnNames []string) error {
    scope, err := scopeFromObjectsWithColumns(db, objects, execFunc, columnNames)
    if err != nil {
        return err
    }
...
func scopeFromObjectsWithColumns(db *gorm.DB, objects []interface{}, execFunc gormbulk.ExecFunc, columnNames []string) (*gorm.Scope, error) {
    // No objects passed, nothing to do.
    if len(objects) < 1 {
        return nil, nil
    }
